...
struct des_proc {
  ...
  natq mq_ntr;
  /// @brief puntatore al buffer/messaggio se il processo è bloccato nella MQ,
  /// nullptr altrimenti
  char *mq_buf;
 ...
 */
natq mq_next(natq idx) { return (idx + 1) % MQ_SIZE; }
...
void mq_msgpush(des_proc *receiver) {
  if (esecuzione->mq_buflen > receiver->mq_buflen)
    return;
  char *src = esecuzione->mq_buf, *dst = receiver->mq_buf;
...
void mq_msgpull(des_proc *sender) {
  if (esecuzione->mq_buflen < sender->mq_buflen)
    return;
  char *dst = esecuzione->mq_buf, *src = sender->mq_buf;
...
extern "C" void c_mq_send(char *msg, natq len) {
  ...
  // ( SOLUZIONE 2025-01-09
  if (esecuzione->mq_reader) {
    flog(LOG_ERR, "mq_send: Processo non registrato come scrittore");
    c_abort_p();
    return;
  }

  if (!c_access(int_cast<vaddr>(msg), len, false, false)) {
    flog(LOG_ERR, "mq_send: Parametri non validi");
    c_abort_p();
    return;
  }

  if (mq_full()) {
    flog(LOG_ERR, "mq_send: message queue piena");
    esecuzione->contesto[I_RAX] = false;
    return;
  }
...
extern "C" void c_mq_recv(char *buf, natq size) {
  ...
  mq_des *mq = &message_queue;
  if (!esecuzione->mq_reader) {
    flog(LOG_ERR, "mq_recv: Processo non registrato come lettore");
    c_abort_p();
    return;
  }

  if (!c_access(int_cast<vaddr>(buf), size, true, false)) {
    flog(LOG_ERR, "mq_recv: Parametri non validi");
    c_abort_p();
    return;
  }
  ...
  mq_msg *msg = &mq->mq[esecuzione->mq_ntr];
  des_proc *sender = msg->sender;
