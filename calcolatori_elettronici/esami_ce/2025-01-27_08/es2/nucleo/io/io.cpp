 ...
 */
natl getpid() { return getmeminfo().pid; }
...
struct ce_des {
  ...
  /// Indice di un semaforo di mutua esclusione nell'accesso al ce_des e alla
  /// periferica
  natl mutex;
};
 ...
 *
 * @param[in,out] current	in: pid dell'ultimo processo servito; out: pid
 * del prossimo processo da servire
 *
 * @return	puntatore alla VCE da servire, o nullptr se tutte le VCE sono
 * vuote
 */
...
extern "C" bool c_vcedbg(natl testid, natl testlen, natb *testdata) {
  ...
  if (curlen != testlen) {
    flog(LOG_WARN, "TEST %u: attesi %u byte, ricevuti %u", testid, testlen,
         curlen);
    return false;
  }
  for (natl i = 0; i < testlen; i++) {
    ...
    if (curb != testb) {
      flog(LOG_WARN, "TEST %u, byte %u: atteso %02x, ricevuto %02x", testid, i,
           testb, curb);
      return false;
    }
  }
...
extern "C" bool c_vcenew() {
  ...
  vce_des *vce = new vce_des;
  if (ce.vces[getpid()]) {
    flog(LOG_ERR, "vcenew: Processo %d ha gia una vce", getpid());
    return false;
  }
...
extern "C" void estern_ce(natq) {
  ...
  // ( SOLUZIONE 2025-01-27
  vce_des *vce = ce.vces[getpid()];
  for (;;) {
    if (evget() == 1) {
      ce.vces[getpid()] = nullptr;
      delete vce;
    }
    natl c = vce_des_read(vce);
    outputb(c, ce.iTBR);
    if (vce->n == 0)
      sem_signal(vce->sync);
    wfi();
...
bool ce_init() {
  ...
  for (natb bus = 0, dev = 0, fun = 0;
    ...
    base &= ~0x1;
    ce.iTBR = base;
    flog(LOG_INFO, "ce: %02x:%02x.%1x base=%04x IRQ=%u", bus, dev, fun, base,
         irq);
    ...
    if (activate_pe(estern_ce, 0, MIN_EXT_PRIO + INTR_TIPO_CE, LIV_SISTEMA, irq,
                    true) == 0xFFFFFFFF) {
