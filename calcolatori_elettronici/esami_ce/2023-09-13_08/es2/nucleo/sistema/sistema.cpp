...
/// @name Solo per debug e test: campi che servono a capire se il page fault è
/// stato gestito
/// @{
...
/**
 * @brief Gestisce i page fault che cadono nella zona cow.
 *
 * Chiamata da gestore_eccezioni() quando l'eccezione è un page fault.
 * Se restiuisce false, gestore_eccezioni() prosegue con la normale gestione
 * dell'eccezione (abort del processo), altrimenti termina.
 *
 * @param v	indirizzo virtuale che ha casuato il fault
 *
 * @return 	true se il fault è stato gestito, false altrimenti
 */
...
extern "C" void gestore_eccezioni(int tipo, natq errore, vaddr rip) {
  ...
  // [ ESAME 2023-09-13
  /// @ingroup esame
  /// @note nel caso di page fault, gestiamo eventuali accessi alla zona cow
...
void init_frame() {
  ...
  // [ ESAME 2023-09-13
  memset(voidptr_cast(primo_frame_libero * DIM_PAGINA), 0xaa,
         N_M2 * DIM_PAGINA);
//   ESAME 2023-09-13 ]
/// @cond
#ifndef N_STEP
...
const vaddr fin_utn_w =
    ini_utn_w + PART_SIZE * N_UTN_W; ///< limite della zona cow
//   ESAME 2023-09-13 )
...
/**
 * @brief Crea la prima copia della zona cow.
 *
 * La zona viene creata prima di creare il primo processo.
 *
 * @param dest 	indirizzo della tabella radice del TRIE in cui creare la zona
 *
 * @return 	false in caso di errore, true altrimenti
 */
bool crea_cow(paddr dest) {
  vaddr v = map(dest, ini_utn_w, ini_utn_w + DIM_USR_COW, BIT_US, [](vaddr v) {
    paddr f = alloca_frame();
    memset(voidptr_cast(f), 0, DIM_PAGINA);
  ...
  if (v != ini_utn_w + DIM_USR_COW) {
    unmap(dest, ini_utn_w, v, [](vaddr, paddr p, int) { rilascia_frame(p); });
    return false;
  }
  return true;
}
...
/**
 * @brief Copia una zona cow da un TRIE a un altro.
 *
 * La copia viene eseguita condividendo tutto il sottoalbero dal livello 3 in
 * giù.
 *
...
bool aggiorna_cow(vaddr v) {
  if (esecuzione->livello != LIV_UTENTE || v < ini_utn_w ||
      v >= ini_utn_w + DIM_USR_COW)
    return false;
...
/**
 * @brief Disfa le azioni di copia_cow() e aggiorna_cow() per il processo
 * corrente.
 */
void distruggi_cow() {
...
des_proc *crea_processo(void f(natq), natq a, int prio, char liv) {
  ...
  // [ ESAME 2023-09-13
  /// @ingroup esame
  /// @note La zona cow del nuovo processo viene copiata dal TRIE attivo.
...
void distruggi_processo(des_proc *p) {
  ...
  // [ ESAME 2023-09-13
  /// @ingroup esame
  /// @note Distruggiamo la zona cow del processo.
  distruggi_cow();
  //   ESAME 2023-09-13 ]
...
extern "C" void main(natq) {
  ...
  // [ ESAME 2023-09-13
  flog(LOG_INFO, "- usr/cow  [%16lx, %16lx)", ini_utn_w, fin_utn_w);
  //   ESAME 2023-09-13 ]
  ...
  // [ ESAME 2023-09-13
  /// @ingroup esame
  /// @note Creiamo la zona cow iniziale.
  if (!crea_cow(init.cr3))
    goto error;
