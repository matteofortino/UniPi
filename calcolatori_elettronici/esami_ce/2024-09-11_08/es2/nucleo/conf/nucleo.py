...
des_proc_std_fields = [ None, 'id', 'cr3', 'contesto', 'livello', 'precedenza', 'puntatore', 'punt_nucleo', 'corpo', 'parametro', 'rws' ]
...
def rw_list(cond='active'):
    max_rw = gdb.parse_and_eval("rw_allocati")
    for i in range(max_rw):
        s = gdb.parse_and_eval("array_desrw[{}]".format(i))
        if cond == 'active' and s['nreaders'] == gdb.Value(0) and s['writer'] == gdb.Value(0) and s['w_readers'] == gdb.Value(0) and s['w_writers'] == gdb.Value(0):
            continue
        yield (i, s.address)

class Rw(gdb.Command):
    """info about rws"""

    def __init__(self):
        super(Rw, self).__init__("rw", gdb.COMMAND_DATA)

    def invoke(self, arg, from_tty):
        a = gdb.string_to_argv(arg)
        if a[0] == "list":
            for i, s in rw_list(*a[1:]):
                gdb.write("rw[{:6}]: {}\n".format(i, str(s)))

Rw()

# ( ESAME
des_rw_ptr_type = gdb.Type.pointer(gdb.lookup_type('des_rw'))

class des_rw_ptrPrinter:
    """Print a des_rw *"""
    ...
    def to_string(self):
        ...
        s = str(self.val.dereference())
        gdb.execute("set print array on")
        gdb.execute("set print pretty on")
        return s

def des_rwLookup(val):
    if val.type == des_rw_ptr_type:
        return des_rw_ptrPrinter(val)
    return None

gdb.pretty_printers.append(des_rwLookup)
