...
struct des_proc {
  ...
  /// id dei semafori da acquisire
  natl mw_sems[MAX_MULTIWAIT];
  /// numero di elementi validi in mw_sems
  natl mw_num;
  /// true se il processo è ombra
  bool shadow;
  //   ESAME 2026-01-09 )
...
/**
 * @brief Conta i semafori multiwait da cui non è possibile estrarre gettoni
 *
 * @param p	processo apartiene la lista multiwait
 *
 * @return 	numero di semafori già occupati
 */
natl mw_missing(des_proc *p) {
  natl missing = p->mw_num;
  for (natl i = 0; i < p->mw_num; i++) {
    des_sem *s = &array_dess[p->mw_sems[i]];
    if (s->counter > 0)
      missing--;
  }
  return missing;
...
des_proc *crea_processo(void f(natq), natq a, int prio, char liv) {
  ...
  // [ ESAME 2026-01-09
  memset(p->mw_sems, 0, sizeof(p->mw_sems));
  p->mw_num = 0;
  p->shadow = false;
  //   ESAME 2026-01-09 ]
...
/// Parte C++ della primitiva sem_multiwait()
extern "C" void c_sem_multiwait(natl *sems, natl n) {
  // ( SOLUZIONE 2026-01-09
  if (n > MAX_MULTIWAIT) {
    c_abort_p();
    return;
  }
  if (!c_access(int_cast<vaddr>(sems), sizeof(natl) * n, false, false)) {
    c_abort_p();
    return;
  }
  memcpy(esecuzione->mw_sems, sems, sizeof(natl) * n);
  for (natl i = 0; i < n; i++) {
    if (!sem_valido(sems[i])) {
      c_abort_p();
      return;
    }
  }
  ...
  if (natl missing = mw_missing(esecuzione)) {
    des_proc *shadows = nullptr;
    for (natl i = 0; i < missing; i++) {
      if (des_proc *ps = new des_proc(*esecuzione)) {
        ps->shadow = true;
        inserimento_lista(shadows, ps);
      } else {
        flog(LOG_WARN, "sem_multiwait: out of memory");
        while (shadows)
          delete rimozione_lista(shadows);
        return;
      }
    }
    for (natl i = 0; i < esecuzione->mw_num; i++) {
      des_sem *s = &array_dess[esecuzione->mw_sems[i]];
      if (s->counter <= 0) {
        s->counter--;
        inserimento_lista(s->pointer, rimozione_lista(shadows));
      }
    }
    schedulatore();
  } else {
