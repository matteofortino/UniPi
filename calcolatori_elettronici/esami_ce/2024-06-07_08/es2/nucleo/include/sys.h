...
/**
 * @brief Installa un watch.
 *
 * @param ptr		puntatore alla variabile da osservare
 * @param size		dimensione della variabile da osservare
 *
 * @pre La variabile deve trovarsi nello spazio utente/condiviso e
 *      deve essere accessibile in lettura.
 * @pre Il puntatore alla variabile deve essere allineato naturalmente.
 * @pre La dimensione della variabile deve essere 1, 2, 4 o 8.
 *
 * @return 		false se c'è già un watch installato, true altrimenti
 */
extern "C" bool setwatch(void *ptr, size_t size);

/**
 * @brief 	Osserva una variabile.
 * @note Se necessario, estende il valore ad 8 byte (senza segno).
 * @pre Ci deve essere un watch installato.
 * @pre Il processo deve essere il watcher.
 * @return 	prossimo valore della varibile osservata
 */
extern "C" natq watch();
//   ESAME 2024-06-07 )
