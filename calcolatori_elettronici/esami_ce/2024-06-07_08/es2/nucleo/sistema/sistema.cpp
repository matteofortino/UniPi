...
extern "C" void gestore_eccezioni(int tipo, natq errore, vaddr rip) {
  ...
  // [ ESAME 2024-06-07
  /// @ingroup esame
  /// @note se l'eccezione è un page fault in scrittura gestiamo eventuali
  /// eventi watch
  if (tipo == 14 && (errore & PF_WRITE) && handle_watch_pf(readCR2()))
    return;

  /// @ingroup esame
  /// @note se l'eccezione è un single step gestiamo eventuali eventi watch
...
des_proc *crea_processo(void f(natq), natq a, int prio, char liv) {
  ...
  // [ ESAME 2024-06-07
  /// @ingroup esame
  /// @note inizializziamo i campi del meccanismo watch
...
void distruggi_processo(des_proc *p) {
  // [ ESAME 2024-06-07
  /// @ingroup esame
  /// @note quando termina un processo watcher dobbiamo resettare
  /// il watch_state
  delwatch(p);
  //   ESAME 2024-06-07 ]
...
/// @brief Descrittore di zona osservata
struct watch_des {
  /// indirizzo virtuale della variabile osservata (se 0, non ci sono watch
  /// installati)
  vaddr v;
  ...
  /// lista su cui si sospendono i processi osservati in attesa di passare il
  /// valore al watcher
  des_proc *watched_waiting;
};

/// @brief istanza globale di watch_des
watch_des watch_state;

/**
 * @brief Inserimento in fondo alla lista.
 *
 * @param p_lista	testa della lista
 * @param p_elem	elemento da inserire
 */
void inserimento_in_fondo(des_proc *&p_lista, des_proc *p_elem) {
  des_proc **p;
  for (p = &p_lista; *p; p = &(*p)->puntatore)
    ;
  *p = p_elem;
  p_elem->puntatore = nullptr;
}
...
/**
 * @brief Parte C++ della primitiva setwatch.
 *
 * @param ptr	puntatore alla zona da osservare
 * @param size	dimensione della zona da osservare
 */
...
/**
 * @brief Disattiva il watch corrente se il watcher termina
 *
 * @param p	processo che sta terminando
 */
void delwatch(des_proc *p) {
  watch_des *w = &watch_state;
...
/**
 * @brief 	Gestisce eventuali eventi watch (page fault)
 *
 * @note Se la funzione viene invocata, il page fault è stato sicuramente
 * causato da un processo (I/O o utente) a causa di una operazione di
 * scrittura.
 *
 * @param cr2	indirizzo virtuale che ha causato il fault
 *
...
bool handle_watch_pf(vaddr cr2) {
  ...
  // permettiamo al processo di eseguire la scrittura e lo catturiamo subito
  // dopo
  natq *pila = ptr_cast<natq>(esecuzione->contesto[I_RSP]);
  pila[2] |= BIT_TF;
  pila[2] &= ~BIT_IF;
...
bool handle_watch_ss() {
  ...
  natq curr = 0;
  memcpy(&curr, voidptr_cast(w->v), w->size);
  ...
  if (w->watcher_waiting) {
    w->watcher_waiting->contesto[I_RAX] = curr;
    inspronti();
    inserimento_lista(pronti, w->watcher_waiting);
    w->watcher_waiting = nullptr;
    schedulatore();
  } else {
    esecuzione->new_watch_value = curr;
  }
