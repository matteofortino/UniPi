...
/**
 * @brief Estrae un gettone da un semaforo, con timeout.
 *
 * @param sem	id del semaforo
 * @param to	timeout
 *
 * @pre il semaforo deve esistere
 * @pre il timeout deve essere maggiore di zero
 *
 * @return 	0 se il timeout è scattato prima che la primitiva
 * 		riuscisse ad estrarre il gettone, altrimenti quanti
 * 		intervalli di tempo mancavano allo scatto del timeout
 * 	 	quando il gettone è stato estratto
 */
extern "C" natl sem_wait_to(natl sem, natl to);

/**
 * @brief Lettura di una sequenza di byte da una periferica CE, con timeout.
 *
 * Se l'operazione complessiva (inclusa l'acquisizione della mutua esclusione)
 * non si conclude entro il timeout, la primitiva interrompe il trasferimento
 * prima che vengano trasmessi tutti i byte richiesti.
 *
 * @param id		id della periferica
 * @param buf		buffer destinato a contenere i byte letti
 * @param[in,out] quanti in: numero di byte da leggere, out: numero di byte
 * 			effettivamente letti
 * @param to		timeout
 *
 * @pre La periferica deve esistere.
 * @pre Non ci devono essere problemi di Cavallo di Troia.
 * @pre Il timeout non deve essere zero.
 *
 * @return 	true se l'operazione si è conclusa prima dello scatto del
 * 		timeout, false altrimenti
 */
extern "C" bool ceread_n_to(natl id, char *buf, natq& quanti, natl to);
	
/// Non usare: solo per debug e testo
extern "C" void cedebug(natl id, const char *buf, natq quanti);
//   ESAME 2024-01-10 )
