...
struct des_proc {
  ...
  /// indice del semaforo su cui il processo è in attesa con timeout (0xffffffff
  /// se nessuno)
  natl waiting_on;
  //   ESAME 2024-01-10 )
...
/**
 * @brief Rimuove un processo dalla coda del timer.
 *
 * @param lavoro	descrittore del processo da rimuovere
 *
 * @return 	tempo che il processo avrebbe dovuto ancora attendere
 * 		nella lista del timer, 0 se non trovato
 */
natl rimozione_lista_attesa(des_proc *lavoro);
//   ESAME 2024-01-10 )
...
extern "C" void c_sem_signal(natl sem) {
  ...
  if (s->counter <= 0) {
    ...
    // [ ESAME 2024-01-10
    /// @note se il processo da risvegliare aveva invocato sem_wait_to() lo
    /// estraiamo dalla lista del timer
...
extern "C" void c_driver_td(void) {
  // [ ESAME 2024-01-10
  // rallentiamo il clock per rendere il sistema più facilmente testabile
  static natq skip = 0;
  ...
  while (sospesi != nullptr && sospesi->d_attesa == 0) {
    // [ ESAME 2024-01-10
    /// @ingroup esame
    /// @note se il processo da risvegliare era bloccato su una sem_wait_to()
    /// la annulliamo (incremento del contatore ed estrazione dalla coda del
    /// semaforo)
...
/// Parte C++ della primitiva sem_wait_to()
...
extern "C" void main(natq) {
  ...
  // [ ESAME 2024-01-10
  /// @ingroup esame
  /// @note Inizializziamo il sottosistema di gestione delle periferiche CE.
  ce_init();
  //   ESAME 2024-01-10 ]
...
extern "C" void c_ceread_n_to(natl id, char *buf, natl &quanti, natl to) {
  if (id > next_ce) {
    c_abort_p();
    return;
  }
  ...
  natl ret;
  ret = sem_wait_to(d->mutex, to);
  d->buf = buf;
  d->quanti = quanti;
  d->enabled = true;
  sem_wait_to(d->sync, to);
  sem_signal(d->mutex);
 ...
 *
 * Per ogni periferica trovata:
 * - alloca e inizializza un descrittore des_ce
 * - abilita le richieste di interruzioni provenienti dalla periferica
 *
...
bool ce_init() {
  ...
  for (natb bus = 0, dev = 0, fun = 0;
       pci::find_dev(bus, dev, fun, 0xedce, 0x1234); pci::next(bus, dev, fun)) {
    if (next_ce >= MAX_CE) {
    ...
    flog(LOG_INFO, "ce%d: %02x:%02x.%1x base=%04x IRQ=%u", next_ce, bus, dev,
         fun, ce->iSTS, irq);
    next_ce++;
  }
  return true;
