...
extern "C" void c_writeconsole(const char *buff, natq quanti) {
  ...
  if (!access(buff, quanti, false, false)) {
    flog(LOG_WARN, "writeconsole: parametri non validi: %p, %lu:", buff,
         quanti);
    abort_p();
  }
 ...
 */
extern "C" void c_iniconsole(natb cc) { vid::clear(cc); }
/// @}
...
bool kbd_init() {
  ...
  if (activate_pe(estern_kbd, 0, MIN_EXT_PRIO + INTR_TIPO_KBD, LIV_SISTEMA,
                  KBD_IRQ) == 0xFFFFFFFF) {
...
bool hd_init() {
  ...
  id = activate_pe(estern_hd, 0, MIN_EXT_PRIO + INTR_TIPO_HD, LIV_SISTEMA,
                   HD_IRQ);
  if (id == 0xFFFFFFFF) {
...
/// Descrittore di trasferimento da periferica CE
struct des_ce_tr {
  /// Destinazione del prossimo byte da leggere (nullptr se il descrittore è
  /// libero)
  char *buf;
  /// Numero di byte ancora da leggere
  natl quanti;
  /// Indice del prossimo trasferimento (0xFFFFFFFF se nessuno)
  natl next;
};
...
struct des_ce {
  ...
  ioaddr iCTL, ///< Registro di controllo
      ...
      iRBR;    ///< Buffer di ingresso
  /// @}

  /// Descrittori di trasferimento
  des_ce_tr tr[MAX_CE_ASYNC];
  /// Indice del trasferimento in corso (0xFFFFFFFF se nessuno)
  natl cur;
  /// Indice dell'ultimo trasferimento richiesto (0xFFFFFFFF se nessuno)
  natl last;
  /// Numero di processi bloccati su cewait
  natl waiting;
  /// Indice di un semaforo di sincronizzazione
  natl sync;
  /// Indice di un semaforo di mutua esclusione
...
extern "C" void c_cewait(natl id, natl trid) {
  if (id >= next_ce) {
    flog(LOG_WARN, "cewait: ce non riconosciuto: %u", id);
    abort_p();
  }

  if (trid >= MAX_CE_ASYNC) {
    flog(LOG_WARN, "cewait: trid non valido: %u", trid);
    abort_p();
  }
...
/// @brief Trova le periferiche CE installate e crea i rispettivi processi
/// esterni
/// @return false se non trova o riesce ad inizializzare nessuna periferica CE,
/// true altrimenti
bool ce_init() {
  ...
  for (natb bus = 0, dev = 0, fun = 0;
    ...
    if (activate_pe(estern_ce, next_ce, MIN_EXT_PRIO + INTR_TIPO_CE + next_ce,
                    LIV_SISTEMA, irq) == 0xFFFFFFFF) {
      flog(LOG_WARN, "ce%d: impossibile attivare processo esterno", next_ce);
      break;
    }
    flog(LOG_INFO, "ce%d: %02x:%02x.%1x base=%04x IRQ=%d", next_ce, bus, dev,
         fun, ce->iSTS, irq);
    next_ce++;
  }
  return next_ce != 0;
...
/*! @brief Riempie i gate della IDT relativi alle primitive fornite dal modulo
 * I/O
 *  @return		true in caso di successo, false altrimenti
 */
...
extern "C" void main_io(natq p) {
  ...
  flog(LOG_INFO, "Heap del modulo I/O: %llxB [%p, %p)", DIM_IO_HEAP, end_,
       end_ + DIM_IO_HEAP);
  ...
  // [ ESAME 2025-09-10
  if (!ce_init()) {
    flog(LOG_ERR, "inizializzaizone di ce fallita");
    abort_p();
  }
  //   ESAME 2025-09-10 ]
  *p_io_init_done = 1;
