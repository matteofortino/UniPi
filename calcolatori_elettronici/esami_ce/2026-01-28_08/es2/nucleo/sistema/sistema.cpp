...
/// Descrittore di RW
struct des_rw {
  	/// id di processi che possiedono il diritto di lettura
	natl readers[MAX_RW_READERS];
	/// id del processo che possiede il diritto di scrittura (0 se nessuno) 
	natl writer;
	/// numero dei processi che possiedono il diritto di lettura
	natl nreaders;
	/// processi in attesa di acquisire il diritto di lettura
	des_proc* w_readers;
	/// processi in attesa di acquisire il diritto di scrittura
	des_proc* w_writers;
};

/// Array dei descrittori di RW
des_rw array_desrw[MAX_RW];
/// Numero di RW allocati
natl rw_allocati = 0;

/**
 * @brief Alloca un nuovo descrittore di RW.
 *
...
/**
 * @brief Controlla la validità di un id di RW.
 *
 * @param rw id da controllare
 *
 * @return  true se l'id corrisponde ad un RW allocato, false altrimenti
 */
...
/**
 * @brief Cerca un processo tra quelli che possiedono il diritto di lettura su un RW.
 *
 * @note Può essere usata anche per cercare una posizione libera nell'array,
 *       passando 0 come id da cercare.
 *
 * @param p	puntatore al descrittore del RW
 * @param r	id del processo da cercare
 *
 * @return indice del processo nell'array readers, 0xFFFFFFFF se non trovato
 */
natl rw_find_reader(des_rw* p, natl r)
{
	for (int i = 0; i < MAX_RW_READERS; i++)
		if (p->readers[i] == r)
			return i;
...
/// Parte C++ della primitiva rw_acq_write()
extern "C" void c_rw_acq_write(natl rw)
{
	if (!rw_valido(rw)) {
		flog(LOG_WARN, "rw_acq_write: rw non valido: %d", rw);
		c_abort_p();
		return;
	}
	...
	if (p->writer == esecuzione->id ||
	    rw_find_reader(p, esecuzione->id) != 0xFFFFFFFF) {
		flog(LOG_WARN, "rw_acq_write: tenta di ri-acquisire un diritto su %d", rw);
		c_abort_p();
		return;
	}
...
{
	if (!rw_valido(rw)) {
		flog(LOG_WARN, "rw_ac_read: rw non valido: %d", rw);
		c_abort_p();
		return;
	}
	...
	if (p->writer == esecuzione->id ||
	    rw_find_reader(p, esecuzione->id) != 0xFFFFFFFF) {
		flog(LOG_WARN, "rw_acq_read: tenta di (ri-)acquisire la lettura su %d", rw);
		c_abort_p();
		return;
	}
...
/// Parte C++ della primitiva rw_rel_write()
extern "C" void c_rw_rel_write(natl rw)
{
	if (!rw_valido(rw)) {
		flog(LOG_WARN, "rw_rel_write: rw non valido: %d", rw);
		c_abort_p();
		return;
	}
	...
	if (p->writer != esecuzione->id) {
		flog(LOG_WARN, "rw_rel_write: tenta di rilasciare la scrittura su %d", rw);
		c_abort_p();
		return;
	}
...
extern "C" void c_rw_rel_read(natl rw)
{
	...
	natl pos = rw_find_reader(p, esecuzione->id);
	if (pos == 0xFFFFFFFF) {
