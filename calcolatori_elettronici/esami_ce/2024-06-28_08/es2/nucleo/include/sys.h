...
/**
 * @brief Crea la relazione master-slave per un checkpoint.
 * @ingroup esame
 *
 * La primitia fallisce se lo slave non esiste, se il processo che invoca
 * la primitiva è già slave, o se qualche allocazione di memoria fallisce.
 *
 * @pre Il processo invocante non deve essere già master.
 * @pre _pid_ deve essere un id di processo valido.
 * @pre Il processo _pid_ deve essere un processo utente.
 * @pre Il processo _pid_ non può essere lo stesso di quello che invoca la primitiva.
 *
 * @param pid		id del processo slave
 *
 * @return		indirizzo della (futura) copia della memoria privata
 * 			dello slave (nullptr in faso di fallimento)
 */
extern "C" void* cp_prep(natl pid);

/**
 * @brief Attende che lo slave trasferisca un checkpoint.
 * @ingroup esame
 *
 * @pre Il processo invocante deve essere master.
 * @pre _regs_ non deve causare problemi di cavallo di troia.
 *
 * @param regs		puntatore all'array che conterrà lo stato dei registri
 *
 * @return 		false se il processo slave è terminato, true altrimenti
 */
extern "C" bool cp_get(natq *regs);
//   ESAME 2024-06-28 )
