...
extern "C" void c_driver_td(void) {
  ...
  while (sospesi != nullptr && sospesi->d_attesa == 0) {
    // [ ESAME 2024-01-29
    /// @ingroup esame
    /// @note Eseguiamo le azioni richieste da un eventuale timeout su una
    /// barriera.
...
void check_barrier(des_proc *p) {
  ...
  while (des_proc *w = rimozione_lista(b->waiting)) {
    w->contesto[I_RAX] = !b->bad;
    if (w == p)
      continue;
...
extern "C" void c_barrier(natl id) {
  ...
  if (!b->first) {
    ...
    r->d_attesa = b->timeout;
    r->pp = esecuzione;
    inserimento_lista_attesa(r);
    esecuzione->contesto[I_RAX] = true;
  }
  ...
  if (b->narrived == b->nproc) {
    ...
    b->first = nullptr;
    while (des_proc *p = rimozione_lista(b->waiting)) {
      p->contesto[I_RAX] = true;
      inserimento_lista(pronti, p);
    }
    b->narrived = 0;
  }
