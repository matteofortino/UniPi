 ...
 * @pre il processo da rimuovere deve essere un processo utente diverso
 *      dal processo che invoca la primitiva;
 * @pre il buffer non deve causare problemi di Cavallo di Troia.
 *
 * @param pid		processo da rimuovere
 * @param buf		puntatore ad un buffer di dimensione DIM_USR_STACK
 *
 * @return		false se il processo non esiste o è già stato rimosso,
 * 			true altrimenti 
 */
 ...
 *
 * Ricrea la parte utente/privata di un processo, inizializzandola
 * con il contenuto del buffer passato come secondo argomento.
 *
 * @pre _pid_ deve essere un identificatore valido di processo;
 * @pre il processo da ricaricare deve essere un processo utente;
 * @pre il buffer non deve causare problemi di Cavallo di Troia.
 *
 * @param pid		processo da ricaricare
 * @param buf		puntatore ad un buffer di dimensione DIM_USR_STACK
 *
 * @return		false se il processo non esiste o è già caricato,
