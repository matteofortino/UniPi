...
/// Stati della parte utente/privata di un processo
enum memstate_t {
  M_OK,         ///< creata e caricata
  M_INCOMPLETE, ///< non creata
  M_SWAPPED_OUT ///< creata, ma rimossa
};

/// Informazioni relative allo swapper
struct des_swapper {
  /// id del processo swapper (0xFFFFFFFF se non registrato)
  natl id;

  /// true se lo swapper è bloccato su get_swap_ev()
  bool waiting;

  /// processi il cui pid deve essere inviato allo swapper
  struct des_proc *tonotify;
};

/// Unica istanza di des_swapper
des_swapper swapper_info = {
    0xFFFFFFFF, // id
    false,      // waiting
    nullptr     // tonotify
};
...
struct des_proc {
  ...
  // [ ESAME 2025-07-16
  /// stato della memoria utente/privata del processo
  memstate_t memstate;

  /// true se il processo è stato saltato dallo schedulatore
  bool scheduled;
  //   ESAME 2025-07-16 ]

  // ( ESAME 2025-07-16

  /// true se il processo è residente
  bool resident;

  /// se !nullptr, c'è uno swap_out in sospeso
  char *swap_out_buf;
  //   ESAME 2025-07-16 )
  ...
  /// parametro `f` passato alla `activate_p`/`_pe` che ha creato questo
  /// processo
  void (*corpo)(natq);
  /// parametro `a` passato alla `activate_p`/`_pe` che ha creato questo
  /// processo
  natq parametro;
  /// @}
};
 ...
 *  @return 		processo a più alta priorità, o nullptr se la lista è
 * vuota
 */
 ...
 *  @return  		true se sem è l'id di un semaforo allocato; false
 * altrimenti
 */
...
///
/// Dispensa:
/// <https://calcolatori.iet.unipi.it/resources/paginazione-implementazione.pdf>
/// @{
/////////////////////////////////////////////////////////////////////////////////
...
#define N_STEP 1
  ...
  for (natq j = 0; j < N_STEP; j++) {
    ...
    vdf[primo_frame_libero + last].prossimo_libero = primo_frame_libero + j + 1;
  }
...
/// Per esempio, la parte sistema/condivisa usa @ref N_SIS_C entrate a partire
/// da
/// @ref I_SIS_C e contiene tutti e soli gli indirizzi la cui traduzione passa
/// da queste entrate.
/// @{
...
const vaddr ini_sis_c =
    norm(I_SIS_C * PART_SIZE); ///< base di sistema/condivisa
const vaddr ini_sis_p = norm(I_SIS_P * PART_SIZE); ///< base di sistema/privata
const vaddr ini_mio_c =
    norm(I_MIO_C * PART_SIZE); ///< base di modulo IO/condivisa
...
const vaddr fin_sis_c =
    ini_sis_c + PART_SIZE * N_SIS_C; ///< limite di sistema/condivisa
const vaddr fin_sis_p =
    ini_sis_p + PART_SIZE * N_SIS_P; ///< limite di sistema/privata
const vaddr fin_mio_c =
    ini_mio_c + PART_SIZE * N_MIO_C; ///< limite di modulo IO/condivisa
const vaddr fin_utn_c =
    ini_utn_c + PART_SIZE * N_UTN_C; ///< limite di utente/condivisa
const vaddr fin_utn_p =
    ini_utn_p + PART_SIZE * N_UTN_P; ///< limite di utente/privata
/// @}
...
void rilascia_tab(paddr f) {
  if (int n = get_ref(f)) {
    fpanic("tentativo di deallocare la tabella %lx con %d entrate valide", f,
           n);
  }
  rilascia_frame(f);
}
 ...
 *  @param f 		indirizzo fisico della tabella
 */
void inc_ref(paddr f) { vdf[f / DIM_PAGINA].nvalide++; }

/*! @brief Decrementa il contatore delle entrate valide di una tabella
 *  @param f 		indirizzo fisico della tabella
 */
void dec_ref(paddr f) { vdf[f / DIM_PAGINA].nvalide--; }

/*! @brief Legge il contatore delle entrate valide di una tabella
 *  @param f 		indirizzo fisico della tabella
 *  @return 		valore del contatore
 */
natl get_ref(paddr f) { return vdf[f / DIM_PAGINA].nvalide; }
/// @}

/*! @brief Controlla che un indirizzo appartenga alla zona utente/condivisa
 *  @param v 		indirizzo virtuale da controllare
 *  @return 		true sse _v_ appartiene alla parte utente/condivisa,
 * false altrimenti
 */
bool in_utn_c(vaddr v) { return v >= ini_utn_c && v < fin_utn_c; }

/*! @brief Controlla che un indirizzo appartenga alla zona sistema/condivisa
 *  @param v 		indirizzo virtuale da controllare
 *  @return 		true sse _v_ appartiene alla parte sistema/condivisa,
 * false altrimenti
 */
bool in_sis_c(vaddr v) { return v >= ini_sis_c && v < fin_sis_c; }
...
natl alloca_proc_id(des_proc *p) {
  ...
  // In questo modo gli id tendono ad essere riutilizzati il più tardi
  // possibile, cosa che aiuta chi deve debuggare i propri programmi
  // multiprocesso.
...
bool crea_pila(paddr root_tab, vaddr bottom, natq size, natl liv) {
  vaddr v = map(root_tab, bottom - size, bottom,
...
void distruggi_pila(paddr root_tab, vaddr bottom, natq size) {
  unmap(root_tab, bottom - size, bottom,
        [](vaddr, paddr p, int) { rilascia_frame(p); });
}
...
des_proc *crea_processo(void f(natq), natq a, int prio, char liv) {
  ...
  if (!crea_pila(p->cr3, fin_sis_p, DIM_SYS_STACK, LIV_SISTEMA)) {
    flog(LOG_WARN, "crea_processo: creazione pila sistema fallita");
    goto err_rel_tab;
  }
...
///
/// - all'ingresso nel sistema (in salva_stato()), salviamo il valore di @ref
/// esecuzione
///   in @ref esecuzione_precedente; questo è il processo a cui appartiene la
///   pila sistema che stiamo usando;
/// - in @ref distruggi_processo(), se @ref esecuzione è uguale a @ref
/// esecuzione_precedente
///   (stiamo distruggendo proprio il processo a cui appartiene la pila
///   corrente), _non_ distruggiamo la pila sistema e settiamo la variabile @ref
///   ultimo_terminato;
/// - in carica_stato(), dopo aver cambiato pila, se @ref ultimo_terminato è
/// settato,
...
/// @brief Se diverso da zero, indirizzo fisico della root_tab dell'ultimo
/// processo
...
/*! @brief Distrugge la pila sistema del processo uscente e rilascia la sua
 * tabella radice.
 *
 ...
 *  @param irq		piedino dell'APIC associato alla richiesta di
 * interruzione
 */
 ...
 */
extern "C" void c_activate_pe(void f(natq), natq a, natl prio, natl liv,
                              natb irq) {
  des_proc *p; // des_proc per il nuovo processo
  ...
  // 'a_p'
  if (irq >= apic::MAX_IRQ) {
    flog(LOG_WARN, "activate_pe: irq %hhu non valido (max %u)", irq,
         apic::MAX_IRQ);
    return;
  }
 ...
 */
extern "C" void c_phys_dealloc(void *ptr) { dealloc(ptr); }
/// @}
/// @}
...
/// creato tramite crea_processo(), ma usa direttamente alcune risorse già
/// allocate. In particolare, usa una pila sistema allocata staticamente e la
/// root table del TRIE creato dal boot loader. Il suo descrittore di processo è
/// allocato direttamente sulla pila sistema. Il processo init resta in vita
/// fino allo shutdown e, una volta completate le inizializzazioni, svolge il
/// ruolo del processo dummy.
///
/// @{
///////////////////////////////////////////////////////////////////////////////////
...
/// @cond
entry_t carica_modulo(boot64_modinfo *mod, paddr root_tab, natq flags,
                      natq heap_size);
/// @endcond
...
/*! @brief Inizializza il sistema e poi esegue il ciclo dummy fino allo
 * shutdown.
 */
extern "C" void main(natq) {
  ...
  // Allochiamo un id per il processo (comparirà nei log da questo momento in
  // poi) Nota: l'id di init, il primo che allochiamo, sarà zero. In questo modo
  // il valore 0 può essere usato come valore non valido nelle strutture dati
  // che devono memorizzare un id di processo (in particolare negli esercizi),
  // in quanto init lo tiene occupato e dunque non assegnabile ad altri
  // processi.
  if ((init.id = alloca_proc_id(&init)) != 0) {
    flog(LOG_ERR, "Errore nell'allocazione del primo id (%u invece di 0)",
         init.id);
    goto error;
  }
  ...
  if ((mio_entry = carica_modulo(&boot_info->mod[1], init.cr3, 0,
                                 DIM_IO_HEAP)) == nullptr)
    goto error;
  flog(LOG_INFO, "Carico il modulo utente");
  if ((utn_entry = carica_modulo(&boot_info->mod[2], init.cr3, BIT_US,
                                 DIM_USR_HEAP)) == nullptr)
    goto error;
  ...
  flog(LOG_INFO, "Heap del modulo sistema: aggiunto [%llx, %llx)", 1 * MiB,
       2 * MiB);
  ...
  main_io = crea_processo(mio_entry, int_cast<natq>(&io_init_done),
                          MAX_EXT_PRIO, LIV_SISTEMA);
  if (main_io == nullptr) {
  ...
  // [ ESAME 2025-07-16
  if (main_utente == nullptr || main_utente->memstate != M_OK) {
    //   ESAME 2025-07-16 ]
...
/// I byte che ora si trovano in _[mod_start + offset, mod_start + offset +
/// size)_ dovranno diventare visibili a partire dall'indirizzo virtuale
/// _vaddr_. Si noti che _memsize_ può essere più grande di _size_, e in quel
/// caso i byte eccedenti devono essere azzerati.
///
 ...
 *  @param flags	BIT_US per rendere il modulo accessibile da livello
 * utente, altrimenti 0
...
entry_t carica_modulo(boot64_modinfo *mod, paddr root_tab, natq flags,
  ...
  for (natq i = 0; i < mod->numseg; i++) {
    ...
    paddr mod_beg = mod->mod_start + s->offset, mod_end = mod_beg + s->size;
    vaddr virt_beg = s->vaddr, virt_end = s->vaddr + s->memsize;
    ...
    if (map(root_tab, virt_beg, virt_end, seg_flags,
    ...
    flog(LOG_INFO, " - segmento %s %s mappato a [%16lx, %16lx)",
         ...
         (seg_flags & BIT_RW) ? "read/write" : "read-only ", virt_beg,
         virt_end);
  }
  ...
  // alloca_frame()
  if (map(root_tab, last_vaddr, last_vaddr + heap_size, flags | BIT_RW,
...
extern "C" void panic(const char *msg) {
  ...
  if (esecuzione_precedente) {
    ...
    flog(LOG_ERR, "------------------------------ PROCESSO IN ESECUZIONE "
                  "-------------------------------");
    flog(LOG_ERR, "corpo %p(%lu), livello %s, precedenza %u", esecuzione->corpo,
         esecuzione->parametro,
 ...
 */
extern "C" void c_nmi() { panic("INTERRUZIONE FORZATA"); }
/// @}
...
void process_dump(des_proc *p, log_sev sev) {
  flog(sev, "proc %u: corpo %p(%lu), livello %s, precedenza %u", p->id,
       p->corpo, p->parametro, p->livello == LIV_UTENTE ? "UTENTE" : "SISTEMA",
       p->precedenza);
...
/**
 * @brief Rimuove la parte utente/privata di un processo.
 *
 * Il contenuto della memoria rimossa viene copiato nel buffer
 * passato come argomento.
 *
 * @param p	processo a cui rimuovere la parte utente/privata
 * @param buf	buffer in cui copiarne il contenuto (deve essere
 * 		grande almeno @ref DIM_USR_STACK byte)
 *
 * @pre  buf deve trovarsi nella memoria virtuale del processo corrente.
 */
void rimuovi_pila(des_proc *p, char *buf) {
  unmap(p->cr3, fin_utn_p - DIM_USR_STACK, fin_utn_p,
        [&](vaddr, paddr pa, int) {
          memcpy(buf, voidptr_cast(pa), DIM_PAGINA);
          rilascia_frame(pa);
          buf += DIM_PAGINA;
        });
}

/// Parte C++ della primitiva swap_out()
extern "C" void c_swap_out(natl pid, char *buf) {
  if (esecuzione->id != swapper_info.id) {
    flog(LOG_WARN, "swap_out: chiamata da processo non-swapper");
    c_abort_p();
    return;
  }
  ...
  if (p->livello != LIV_UTENTE) {
    flog(LOG_WARN, "swap_out: processo %u non è di livello utente", pid);
    c_abort_p();
    return;
  }
  ...
  /// @todo: completare swap_out()
  // ( SOLUZIONE 2025-07-16

  if (p->resident) {
    swapper_info.waiting = true;
    p->swap_out_buf = buf;
    // inserimento_lista(swapper_info.tonotify, p);
    schedulatore();
  } else {
    p->swap_out_buf = nullptr;
    swapper_info.waiting = false;
...
/// Parte C++ della primitiva swapper()
extern "C" void c_swapper() {
  if (swapper_info.id != 0xFFFFFFFF) {
    flog(LOG_WARN, "swapper: processo swapper gia' registrato");
    c_abort_p();
    return;
  }
  swapper_info.id = esecuzione->id;
}

extern "C" void c_resident(bool res) {
  /// @todo realizzare la primitiva resident
  // ( SOLUZIONE 2025-07-16
  if (esecuzione->resident == res)
    return;
  if (res) {
    esecuzione->resident = res;
    return;
  }
  ...
  des_proc *swapper = proc_table[swapper_info.id];
  rimuovi_pila(esecuzione, esecuzione->swap_out_buf);
  esecuzione->memstate = M_SWAPPED_OUT;
  esecuzione->swap_out_buf = nullptr;

  inspronti();
  inserimento_lista(pronti, swapper);
  schedulatore();
...
// [ ESAME 2025-07-16
/// Parte C++ della primitiva swap_in()
extern "C" void c_swap_in(natl pid, const char *buf) {
  ...
  case M_SWAPPED_OUT:
    ...
    vaddr v = map(p->cr3, fin_utn_p - DIM_USR_STACK, fin_utn_p, BIT_US | BIT_RW,
                  [&](vaddr) {
                    paddr pa = alloca_frame();
                    if (pa) {
                      memcpy(voidptr_cast(pa), buf, DIM_PAGINA);
                      buf += DIM_PAGINA;
                    }
                    return pa;
                  });
    if (v != fin_utn_p) {
      unmap(p->cr3, fin_utn_p - DIM_USR_STACK, v,
            [](vaddr, paddr pa, int) { rilascia_frame(pa); });
      return;
    }
    break;
  }
  ...
  if (p->scheduled) {
    p->scheduled = false;
    for (des_proc **scan = &swapper_info.tonotify; *scan;
         scan = &(*scan)->puntatore) {
      if ((*scan)->id == p->id) {
        *scan = (*scan)->puntatore;
        break;
...
/// Parte C++ della primitiva get_swap_ev()
extern "C" void c_get_swap_ev() {
  if (esecuzione->id != swapper_info.id) {
    flog(LOG_WARN, "get_swapper_event: invocazione da processo non-swapper");
    c_abort_p();
    return;
  }
  if (swapper_info.tonotify) {
    esecuzione->contesto[I_RAX] = swapper_info.tonotify->id;
  } else {
    swapper_info.waiting = true;
    schedulatore();
  }
