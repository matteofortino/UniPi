...
/**
 * @brief Registra il processo come swapper.
 *
 * @pre lo swapper non deve essere già registrato.
 */
extern "C" void swapper();

/**
 * @brief Riceve il prossimo messaggio per lo swapper.
 *
 * Ogni messagio è il pid di un processo che è stato schedulato
 * mentre la sua memoria utente/privata non era presente
 * (o perché non era stato possibile crearla al momento
 * della activate_p(), o perché lo swapper stesso l'aveva
 * rimossa tramite swap_out()).
 *
 * Se non ci sono processi in questa condizione, la primitiva
 * sospende lo swapper.
 *
 * @pre il processo che invoca deve essere lo swapper;
 * @return il pid di un processo schedulato senza memoria utente/privata
 */
extern "C" natl get_swap_ev();

/**
 * @brief Rimuove un processo dalla memoria.
 *
 * Elimina la parte utente/privata di un processo e ne copia
 * il contenuto nel buffer passato come secondo argomento.
 * Il processo così rimosso diventa non eseguibile fino a
 * quando non viene nuovamente caricato in memoria.
 * Se il processo era incompleto, la primitiva non fa niente.
 *
 * @pre il processo che invoca deve essere lo swapper;
 * @pre _pid_ deve essere un identificatore valido di processo;
 * @pre il processo da rimuovere deve essere diverso
 *      dal processo che invoca la primitiva;
 * @pre se usato, il buffer non deve causare problemi di Cavallo di Troia.
 *
 * @param pid		processo da rimuovere
 * @param buf		puntatore ad un buffer di dimensione DIM_USR_STACK
 *
 * @return		false se il processo non esiste o è di livello sistema,
 * 			true altrimenti 
 */
extern "C" bool swap_out(natl pid, char *buf);

/**
 * @brief Ricarica in memoria un processo precedentemente rimosso.
 *
 * Crea la parte utente/privata di un processo. Se il processo
 * era stato creato in modo incompleto, crea una pila vuota.
 * Altrimenti ricrea la pila utilizzando il contenuto del buffer
 ...
 * @pre se usato, il buffer non deve causare problemi di Cavallo di Troia.
 *
 * @param pid		processo da ricaricare
 * @param buf		puntatore ad un buffer di dimensione DIM_USR_STACK
 *
 * @return		false se il processo non esiste o è di livello sistema,
 * 		        o se l'operazione fallisce; true altrimenti
 */
extern "C" bool swap_in(natl pid, const char *buf);
