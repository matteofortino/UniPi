...
{
	/// @todo implementare lo schedulatore modificato che sceglie
	/// solo processi in stato M_OK e notifica lo swapper negli altri casi
...
{
	// [ ESAME 2025-06-25
	void estrazione_lista(des_proc * &head, natl id);
	if (swapper_info.id == p->id)
	{
		// se il processo swapper termina, lo stato dei processi
		// rimossi potrebbe non essere più recuperabile e una
		// soluzione potrebbe essere quella di distruggerli. Qui
		// ci limitiamo a distruggere quelli che sono creati
		// dal programma di test. Una soluzione generale dovrebbe
		// tener conto del fatto che questi processi potrebbero
		// essere bloccati su un semaforo o sospesi.
		swapper_info.id = 0xFFFFFFFF;
		swapper_info.waiting = false;
		for (natl i = 0; i < MAX_PROC; i++)
		{
			if (des_proc *q = proc_table[i])
			{
				if (q->memstate == M_SWAPPED_OUT)
				{
					estrazione_lista(swapper_info.tonotify, q->id);
					estrazione_lista(pronti, q->id);
					distruggi_processo(q);
					processi--;
				}
			}
		}
...
entry_t carica_modulo(boot64_modinfo *mod, paddr root_tab, natq flags, natq heap_size)
{
	...
	if (map(root_tab,
					...
					flags | BIT_RW,
					[](vaddr)
					{ return alloca_frame(); }) != last_vaddr + heap_size)
		return nullptr;
...
/**
 * @brief Ripristina la parte utente/privata di un processo.
 *
 * Il contenuto della memoria ripristinata è copiato dal
 * buffer passato come secondo argomento.
 *
 * @param p	processo a cui ripristinare la parte utente/privata
 * @param buf	buffer da cui copiarne il contenuto (deve essere
 * 		grande almeno @ref DIM_USR_STACK byte)
 *
 * @return 	false se non è stato possibile riallocare la memoria,
 * 		true altrimenti
 */
bool ripristina_pila(des_proc *p, const char *buf)
{
...
/**
 * @brief Rimuove un processo da una qualunque posizione in una lista
 *
 * @note Se il processo non è in lista, non fa niente
 *
 * @param head	puntatore alla testa della lista
 * @param id	id del processo da rimuovere
 */
void estrazione_lista(des_proc *&head, natl id)
{
	for (des_proc **scan = &head; *scan; scan = &(*scan)->puntatore)
	{
		if ((*scan)->id == id)
		{
			*scan = (*scan)->puntatore;
...
extern "C" void c_swap_in(natl pid, const char *buf)
{
	...
	/// @todo completare la primitiva swap_in()
	// ( SOLUZIONE 2025-06-25
	//   SOLUZIONE 2025-06-25 )
}
