...
bool cfi_backstep(cfi_d& cfi, vaddr rip, bool& was_fault)
{
	...
	while (scan - cfi.eh_frame < cfi.eh_frame_len) {
		natl length = *ptr_cast<natl>(scan);
		scan += 4;
		natl id = *ptr_cast<natl>(scan);
		if (!id) {
			// CIE
			vaddr cie_scan = scan + 5;
			cie_end = scan + length;
			const char* as = ptr_cast<const char>(cie_scan);
			for (int i = 0; as[i]; i++)
				cie_scan++;
			...
			fde_range_beg = fde_scan + *ptr_cast<int>(fde_scan);
			fde_scan += 4;
			vaddr len = *ptr_cast<int>(fde_scan);
			fde_scan += 4;
