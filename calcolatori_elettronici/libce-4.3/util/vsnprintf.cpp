...
static char *uconv(char *buf, unsigned long long v, int base, int ucase)
{
	char *p = buf + BUFSZ;
...
static char *iconv(char *buf, long long v, int base, int ucase)
{
	char *p;
	...
	while (*fmt != '\0') {
		...
		case 'i':
			...
			case 'h':
				...
				break;
			case 'l':
				iscalar = doubled ? va_arg(ap, long long):
						    va_arg(ap, long);
				break;
			case 'j':
		...
		case 'X':
			...
			case 'h':
				...
				break;
			case 'l':
				uscalar = doubled ? va_arg(ap, unsigned long long):
