...
des_msgbox MSGBOX[MAX_MSGBOX];
natl msgbox_allocate = 0;

natl alloca_msgbox() {
  natl nuova_box = msgbox_allocate;
  des_msgbox *box = &MSGBOX[nuova_box];
  box->waiting = 0;
  box->messages = 0;

  msgbox_allocate = msgbox_allocate + 1;
  return nuova_box;
}
...
extern "C" void c_msgbox_init() {
  natl box = alloca_msgbox();
  if (box >= MAX_MSGBOX) {
    flog(LOG_ERR, "Allocate troppe msgbox");
    c_abort_p();
    return;
  }
  flog(LOG_INFO, "[c_msgbox_init] allocata msgbox %u", box);
  esecuzione->contesto[I_RAX] = box;
}
...
extern "C" void c_msgbox_send(natl box_id, natl msg_content) {
  ...
  if (!box->messages) {
    ...
    des_msg *p;
    for (p = box->messages; p && p->next; p = p->next)
      ;
    p->next = msg;
  }
