...
bool scritto = false;
natq messaggio;
des_proc* processi_sospesi = 0;

extern "C" void c_leggi(natq arg){
	if(!scritto){
		inserimento_lista(processi_sospesi, esecuzione);
		schedulatore();
	} else {
		esecuzione->contesto[I_RAX] = messaggio;
	}
...
extern "C" void c_scrivi(natq arg){
	...
	while(processi_sospesi != 0){
		des_proc* p = rimozione_lista(processi_sospesi);
		p->contesto[I_RAX] = arg;
		inserimento_lista(pronti, p);
	}
	scritto = true;
	messaggio = arg;
...
extern "C" void c_pulisci() {
	if(processi_sospesi != 0) {
		flog(LOG_ERR, "ci sono processi sospesi");
		c_abort_p(); 
		return;
	}
	scritto = false;
