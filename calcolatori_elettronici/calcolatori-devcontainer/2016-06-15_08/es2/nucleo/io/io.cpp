...
extern "C" void c_cedmaread(natl id, char *buf, natl quanti)
{
	if (id > next_ce)
	{
		flog(LOG_ERR, "Dispositivo non valido");
		abort_p();
	}

	if (!access(buf, quanti, true))
	{
		flog(LOG_ERR, "buffer non tuo");
		abort_p();
	}
...
extern "C" void extern_ce(natq id)
{
	des_ce *c = &array_ce[id];
...
bool ce_init()
{
		 ...
		 pci::next(bus, dev, fun))
	{
		...
		if (activate_pe(extern_ce, next_ce, MIN_EXT_PRIO + INTR_TIPO_CE + next_ce, LIV_SISTEMA, irq) == 0xFFFFFFFF)
		{
